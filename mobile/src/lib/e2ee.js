 ...
 * `lib/e2ee.js`, and deliberately a near-literal port of it.
 *
 * Every message is encrypted **once** with a random content key (CEK). That CEK
 * is then sealed separately for each recipient, in two flavours:
 *
 ...
 *
 * Decryption tries the device slot first and falls back to the account slot, so
 * a missing or stale session degrades gracefully instead of losing a message.
 */

const ACCOUNT_SLOT = 'account';

/** Unlocked key material for this launch. Never persisted in plain form. */
let state = {
  userId: null,
  deviceId: null,
...
/** Persists unlocked keys for this device and loads them into memory. */
...
/** Rehydrates key material on a cold start. */
...
/**
 * Encrypts a file picked from the device.
 *
 * The browser hands `encryptFile` a File object; here the input is a content
 * URI, so the bytes are read through expo-file-system. Base64 is the only
 * binary encoding it offers, which costs a third in memory — acceptable for
 * photos and documents, and the reason large videos should be streamed rather
 * than sent through this path.
 */
export async function encryptFile({ uri, name, mimeType, size }) {
  const key = await C.generateAesKey();

  const base64 = await FileSystem.readAsStringAsync(uri, {
    encoding: FileSystem.EncodingType.Base64,
  });
  const bytes = C.fromB64(base64);
  const { ciphertext, iv } = await C.aesEncrypt(key, bytes);

  return {
    // Ready to hand to FormData as a file part.
    upload: { uri, base64: ciphertext },
    ciphertext,
    key: C.toB64(key),
    iv,
    meta: { name: name || 'file', mime: mimeType || 'application/octet-stream', size: size ?? bytes.length },
  };
}

/** Downloads, decrypts and caches an attachment, returning a local file path. */
export async function decryptFile({ url, key, iv, mime }) {
  const cached = await vault.getMedia(url);
  if (cached) return cached.path;

  const absolute = url.startsWith('http') ? url : API_ORIGIN + url;
  ...
  return vault.cacheMedia(url, plain, mime, C.toB64);
}
